...
public class Categ2SINC {
	...
	public static ArrayList<weblayer.vendas.DTO.Categ2DTO> RetornaLista(Context context, String SOAP_ADDRESS, int empresa, String vendedor, String dispositivo, String datasinc) throws Exception {
		
		String OPERATION_NAME="RetornaCateg2";
		...
		ArrayList<weblayer.vendas.DTO.Categ2DTO> Objetos;
		
		try
		{
		    
			Type collectionType = new TypeToken<ArrayList<weblayer.vendas.DTO.Categ2DTO>>(){}.getType();
	...
	public static void Sincronizar(Context context, String SOAP_ADDRESS, int id_empresa, String Vendedor, String Imei, String UltimaSinc) throws Exception
	{
		...
		ArrayList<Categ2DTO> itens = RetornaLista(context, SOAP_ADDRESS, id_empresa, Vendedor, Imei, UltimaSinc);
		
		if (itens==null)
			return;
		
		for (Categ2DTO item : itens) {
			
			Categ2DTO buffer = Categ2DAO.Get(item.getid());
			if (buffer==null)
				Categ2DAO.insert(item);
			else
				Categ2DAO.update(item);
