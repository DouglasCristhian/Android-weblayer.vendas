...
public class ProdutoSINC {
	...
	public static ArrayList<weblayer.vendas.DTO.ProdutoDTO> RetornaLista(Context context, String SOAP_ADDRESS, int empresa, String vendedor, String dispositivo, String datasinc) throws Exception {
		
		String OPERATION_NAME="RetornaProdutos";
		...
		ArrayList<weblayer.vendas.DTO.ProdutoDTO> Objetos;
		
		try
		{
		    
			Type collectionType = new TypeToken<ArrayList<weblayer.vendas.DTO.ProdutoDTO>>(){}.getType();
	...
	public static void Sincronizar(Context context,  String SOAP_ADDRESS, int id_empresa, String Vendedor, String Imei, String UltimaSinc) throws Exception
	{
		...
		ArrayList<ProdutoDTO> itens = RetornaLista(context, SOAP_ADDRESS, id_empresa, Vendedor, Imei, UltimaSinc);
		...
		for (ProdutoDTO item : itens) {
			...
			ProdutoDTO buffer = ProdutoDAO.Get(item.getid());
			if (buffer==null)
