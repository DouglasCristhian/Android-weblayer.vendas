...
public class Categ1SINC {
	...
	public static ArrayList<weblayer.vendas.DTO.Categ1DTO> RetornaLista(Context context, String SOAP_ADDRESS, int empresa, String vendedor, String dispositivo, String datasinc) throws Exception {
		
		String OPERATION_NAME="RetornaCateg1";
		...
		ArrayList<weblayer.vendas.DTO.Categ1DTO> Objetos;
		
		try
		{
		    
			Type collectionType = new TypeToken<ArrayList<weblayer.vendas.DTO.Categ1DTO>>(){}.getType();
	...
	public static void Sincronizar(Context context, String SOAP_ADDRESS,  int id_empresa, String Vendedor, String Imei, String UltimaSinc) throws Exception
	{
		...
		ArrayList<Categ1DTO> itens = RetornaLista(context, SOAP_ADDRESS, id_empresa, Vendedor, Imei, UltimaSinc);
		
		if (itens==null)
			return;
		
		for (Categ1DTO item : itens) {
			
			Categ1DTO buffer = Categ1DAO.Get(item.getid());
			if (buffer==null)
				Categ1DAO.insert(item);
			else
				Categ1DAO.update(item);
